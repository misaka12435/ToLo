...
def edge_loss(attn_map, mask, iou):
    ...
    sobel_sum = torch.sqrt(sobel_output_y ** 2  + sobel_output_x ** 2 + 1e-8)
    sobel_sum = sobel_sum 
...
def compute_sep(attn_norm_threshold_list):

    sep_loss=0
    num=0
    for i in range(len(attn_norm_threshold_list)):
        for j in range(i+1,len(attn_norm_threshold_list)):
            if (attn_norm_threshold_list[j][0].sum()==0 or attn_norm_threshold_list[i][0].sum()==0): continue
            sep_loss+=((attn_norm_threshold_list[i][1] * attn_norm_threshold_list[j][0]).sum().detach() * (1 / attn_norm_threshold_list[j][0].sum()))
            sep_loss+=((attn_norm_threshold_list[i][1] * attn_norm_threshold_list[j][0]).sum() * (1 / attn_norm_threshold_list[j][0].sum().detach()))
            sep_loss+=((attn_norm_threshold_list[j][1] * attn_norm_threshold_list[i][0]).sum().detach() * (1 / attn_norm_threshold_list[i][0].sum()))
            sep_loss+=((attn_norm_threshold_list[j][1] * attn_norm_threshold_list[i][0]).sum() * (1 / attn_norm_threshold_list[i][0].sum().detach()))
            num+=2


    if (num==0) : return torch.tensor(0).float().cuda() if torch.cuda.is_available() else torch.tensor(0).float() 
    else : return sep_loss/num

def compute_rnb(attn_maps_mid, attn_maps_up, attn_maps_down, attn_self, bboxes, object_positions, index=None, attn_weight=None,stage1_step=0):
    ...
    for obj_idx in range(object_number):
        ...
        attn_norm_threshold=attn_norm*thres_image  


        attn_norm_threshold_list.append([attn_norm_threshold,thres_image])

        if (index < stage1_step):
    ...
    if (index < stage1_step): loss += obj_loss / object_number
    else: loss+=compute_sep(attn_norm_threshold_list)
