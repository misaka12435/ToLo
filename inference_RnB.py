...
def inference(device, unet, vae, tokenizer, text_encoder, prompt, bboxes, phrases, cfg, logger, p_len=77):
    ...
    for index, t in enumerate(tqdm(noise_scheduler.timesteps)):
        ...
        while iteration < cfg.inference.max_iter and index < cfg.inference.max_index_step + 2:
            ...
            res = compute_rnb(attn_map_integrated_mid, attn_map_integrated_up, attn_maps_down=attn_map_integrated_down, \
                                       attn_self=attn16, bboxes=bboxes, object_positions=object_positions, index=index, attn_weight=attn_weight, stage1_step=cfg.inference.max_index_step) #* cfg.inference.loss_scale 
...
def prepro(rect,phrases):
    phrase=""
    rec=[]
    for i in range(len(phrases)):
        if (len(phrases[i])==2): 
            if (phrases[i][1][-1]=="." or phrases[i][1][-1]==","): phrase+=phrases[i][0]+" "+phrases[i][1][:-1]+";"
            else : phrase+=phrases[i][0]+" "+phrases[i][1]+";"
        else :
            if (phrases[i][0][-1]=="." or phrases[i][0][-1]==","): phrase+=phrases[i][0][:-1]+';'
            else : phrase+=phrases[i][0]+';'
        rec.append(rect[i])
    return rec,phrase[:-1]
...
def main(cfg):
    ...
    for i in range(len(data["prompts"])):
        example={}
        example["prompt"]=data["prompts"][i]
        example["phrases"]=data["sets"][i]
        example["bboxes"]=data["boxes"][i]
        example["bboxes"],example["phrases"]=prepro(example["bboxes"],example["phrases"])
    ...
    for examples in examples_list:
        ...
        if os.path.exists(os.path.join(cfg.general.save_path, f"{str(num).zfill(5)}_{str(0).zfill(2)}_{examples['prompt']}.png")): continue
        ...
        for index, pil_image in enumerate(pil_images):
            image_path = os.path.join(cfg.general.save_path, f"{str(num).zfill(5)}_{str(0).zfill(2)}_{examples['prompt']}.png")
